...
// GotErrorAnyLine reports an error when each chunk produces at most one error.
// It matches the error against the sole remaining expectation regardless of the
// runtime line reported by the evaluator.
func (chunk *Chunk) GotErrorAnyLine(msg string) {
	if len(chunk.wantErrs) != 1 {
		chunk.GotError(1, msg)
		return
	}
