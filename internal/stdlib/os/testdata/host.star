...
assert.eq(type(os.environ), "os._Environ")
assert.true("get" in dir(os.environ))
assert.eq(os.environ.get("DYSON_OS_TEST"), "before")
assert.eq(os.environ.get("DYSON_OS_MISSING"), None)
assert.eq(os.environ.get("DYSON_OS_MISSING", "fallback"), "fallback")
environment_copy = dict(os.environ)
assert.eq(environment_copy["DYSON_OS_TEST"], "before")
assert.eq(os.environ["DYSON_OS_TEST"], "before")
assert.true("DYSON_OS_TEST" in os.environ)
assert.true("DYSON_OS_TEST" in list(os.environ))
assert.eq(len(os.environ), len(environment_copy))
assert.eq(os.environ()["DYSON_OS_TEST"], "before")
os.putenv("DYSON_OS_TEST", "after")
assert.eq(os.getenv("DYSON_OS_TEST"), "after")
assert.eq(os.environ.get("DYSON_OS_TEST"), "after")
assert.eq(os.environ["DYSON_OS_TEST"], "after")
assert.eq(environment_copy["DYSON_OS_TEST"], "before")
os.unsetenv("DYSON_OS_TEST")
assert.eq(os.getenv("DYSON_OS_TEST"), None)
assert.eq(os.environ.get("DYSON_OS_TEST"), None)
assert.eq("DYSON_OS_TEST" in os.environ, False)
