...
// moduleStart is the fallback monotonic origin used when a builtin is called
// without a Starlark thread.
...
// elapsed seconds from the current Starlark thread's monotonic origin.
//
...
// returning elapsed nanoseconds from the current Starlark thread's monotonic
// origin.
//
...
// monotonicStartKey stores the per-thread monotonic origin in starlark.Thread
// locals so each independent execution starts elapsed monotonic time at zero.
const monotonicStartKey = "github.com/spachava753/dyson/internal/stdlib/time.monotonicStart"

// monotonicStart returns the monotonic origin for the current Starlark thread,
